...
proc readInfo(module, stream, ptrs) {.inline.} =
    ...
    for i in 0..31:
        ...
        if inChip != chEnd:
            if $inChip == "":
                raise newException(NotImplementedError, fmt"Unimplemented chip type {ord(inChip)}!")
            module.chips.add(
                ChipInfo(kind:inChip)
            )
    for i in 0..31:
    ...
    for channel in 0 ..< numChannels:
        ...
        for order in 0 ..< int(orderLen):
            module.order[channel].add(
                int(stream.read(uint8))
            )
...
proc readInstruments(module, stream, ptrs) {.inline.} =
    ...
    for i in ptrs.insPtr:
        ...
        of "INST":
            # TODO
            raise newException(NotImplementedError, "The <127 instrument format is not implemented, sorry :(")
        of "INS2":
            module.instruments.add(
                stream.instrumentDev127FromStream(insCounter)
            )
            insCounter += 1
        else:
...
proc readPattern(module, stream, ptrs) {.inline.} =
    for i in ptrs.patternPtr:
        stream.setPosition(i)
        module.patterns.add(
            stream.patternFromStream(module)
        )
...
proc moduleFromFile*(inFileName: string): Module =
    ...
    try:
        furIn = newMemStream(
            cast[seq[byte]](readFile(inFileName).uncompress),
            littleEndian
        )
    except ZippyError:
        # this must be an uncompressed file, then
        furIn = newMemStream(
            cast[seq[byte]](readFile(inFileName)),
            littleEndian
        )
