...
proc readPattern(pattern, stream, module) =
  ...
  for p in 0 ..< patternLength:
    ...
    for f in 0 ..< numEffects:
      newRow.effects.add((stream.read(int16), stream.read(int16)))
...
proc readPattern157(pattern, stream, module) =
  ...
  var patnContents: MemStream =
    newMemStream(cast[seq[byte]](stream.readStr(patnContentSize)), littleEndian)
  ...
  let blankRow = Row(note: nBlank, instrument: -1, volume: -1)
  ...
  while (var patByte = patnContents.read(uint8); patByte) != 0xff:
    ...
    if ptnHasNote in patCmd0:
      ...
      of 182:
        ...
        raise newException(
          ValueError,
          fmt"Unknown note value at subsong {pattern.subsong}, ch {pattern.channel}, index {pattern.index} row {rowsSoFar}",
        )
