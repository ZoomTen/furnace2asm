...
proc readDev127Feature(instrument, stream): Ins2Feature =
  ...
  try:
    result = Ins2Feature(code: parseEnum[Ins2FeatureCode](featureCode))
  except ValueError:
    let message: string =
      case featureCode
    ...
    raise newException(
      ValueError,
      "Unknown feature code \"" & featureCode & "\" in instrument " &
        instrument.index.toHex(2) & " (" & message & ")",
    )
  if result.code == fcEnd:
    return result
  let insContentSize = stream.read(uint16)
  var insContents: MemStream =
    newMemStream(cast[seq[byte]](stream.readStr(insContentSize)), littleEndian)
  case result.code
  of fcName:
  ...
  of fcNesDpcm: # XXX IGNORED
    let sampleMap = insContents.read(uint8).int
    if sampleMap != 0:
      for i in 1 .. 120:
  ...
  of fcMacro:
    ...
    while true:
      var newMacro = Ins2Macro(kind: Ins2MacroCode(insContents.read(uint8)))
      case newMacro.kind
      of mcStop:
