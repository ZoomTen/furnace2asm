import wNim/[wApp, wFrame, wMacros, wPanel, wWebView, wButton]

type wHelpDialog* = ref object of wFrame
  canvas: wPanel
  btnOk: wButton
  txHelp: wWebView

const htHelp = staticRead("images/helpText.html")
...
wClass(wHelpDialog of wFrame):
  proc init(self: wHelpDialog, owner: wWindow) =
    ...
    self.btnOk = self.canvas.Button(label = "Got it!")
