import
  wNim/[
    wApp, wFrame, wMacros, wImage, wButton, wRegion, wPaintDC, wMemoryDC, wBitmap,
    wBrush, wPanel, wStaticText, wFont, wTextCtrl, wFileDialog, wIcon, wCheckBox,
    wMessageDialog,
  ]
...
func withinRange(range: AreaRange, x, y: int): bool {.inline.} =
  (x >= range.x) and (x < (range.x + range.w)) and (y >= range.y) and
    (y < (range.y + range.h))
...
wClass(wMyAppFrame of wFrame):
  ...
  proc init(self: wMyAppFrame) =
    ...
    # setup frame info
    wFrame(self).init(
      title = "Furnace2ASM", size = self.imgBackground.size, style = wClipChildren
    )
      ...
      if e.leftDown():
        ...
        if rPos.y < 228: # limit of top bar
          if not posIsSaved:
            savedPos = rPos
            posIsSaved = true
          self.position = (sPos.x - savedPos.x, sPos.y - savedPos.y)
      else:
        posIsSaved = false
    ...
    self.canvas.wEvent_LeftDown do(e: wEvent):
      ...
      # minimize buttton
      if AreaRange(x: 519, y: 96, w: 23, h: 23).withinRange(pos[0], pos[1]):
        self.minimize()

      # close button
      if AreaRange(x: 554, y: 96, w: 23, h: 23).withinRange(pos[0], pos[1]):
        self.close()
    ...
    self.canvas.connect(WM_ERASEBKGND) do(e: wEvent):
      let hdc = e.getwParam
      hdc.BitBlt(
        0, 0, self.imgBackground.width, self.imgBackground.height, self.dcContext.mHdc,
        0, 0, SRCCOPY,
      )
      e.mResult = 1
  ...
  proc openFur(self: wMyAppFrame) =
    let dlg = self.FileDialog(
      wildcard = "Furnace project (*.fur)|*.fur", style = wFdOpen or wFdFileMustExist
    )
  ...
  proc openAsm(self: wMyAppFrame) =
    let dlg = self.FileDialog(
      wildcard = "Assembly source (*.asm)|*.asm", style = wFdSave or wFdOverwritePrompt
    )
    if dlg.showModal() == wIdOk:
      var path = dlg.path()
  ...
  proc doConvert(self: wMyAppFrame) =
    if self.wOpenedFur.strip() == "":
      ...
      try:
        ...
        discard self
          .MessageDialog(message = e.msg, caption = fmt"{e.name}", style = wIconErr)
          .showModal()
  ...
  proc initControls(self: wMyAppFrame) =
    block addHeaders:
      self.lblSourceFurHeader = self.canvas.StaticText(
        label = "1. Open your module file", pos = (160, 260), size = (400, 20)
      )
    ...
    block addButtons:
      self.btnOpenFur =
        self.canvas.Button(label = "&Open", pos = (160, 285), size = (75, 25))
      self.btnOpenAsm =
        self.canvas.Button(label = "&Save", pos = (160, 345), size = (75, 25))
      self.btnConvert =
      ...
      self.btnAbout =
        self.canvas.Button(label = "About", pos = (35, 320), size = (100, 40))
      self.btnExit =
        self.canvas.Button(label = "Exit", pos = (35, 380), size = (100, 40))
    ...
    block addMisc:
      self.chkUseOldStyle = self.canvas.CheckBox(
        label = "Use old pret macros", pos = (160, 385), size = (305, 24)
      )
